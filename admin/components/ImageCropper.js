...
/**
 * Canvas-based image cropper (no external dependency).
 *
 * Props:
 ...
 *   - outputSize: number (default 1600 — max dimension)
 *   - defaultAspect: number|null (default 16/9)
 */
export default function ImageCropper({
  ...
  const [viewport, setViewport] = useState({ w: 320, h: 180 });
  ...
  const handleConfirm = () => {
    ...
    const outH = aspect == null ? Math.round((outputSize * vh) / vw) : Math.round(outputSize / aspect);
    canvas.width = outW;
    canvas.height = outH;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60" onClick={onCancel}>
      <div className="bg-white rounded-2xl w-full max-w-lg p-5 shadow-2xl space-y-4" onClick={e => e.stopPropagation()}>
        ...
        >
          ...
          >
            ...
            <img src={src} alt="Crop preview" className="max-w-none" draggable={false} style={{ width: imgW, height: imgH }} />
          </div>
