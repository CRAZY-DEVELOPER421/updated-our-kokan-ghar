...
function SectionEditor({ section, index, onUpdate, onDelete, onMoveUp, onMoveDown, onDragStart, onDragOver, onDragEnd, onDragLeave, isDragging, isFirst, isLast }) {
  const renderEditor = () => {
    switch (section.type) {
      case 'heading':
        return (
      ...
      case 'image':
        return (
          <div className="space-y-2">
            ...
            <input className="w-full border rounded-lg px-3 py-2 text-sm" value={section.caption || ''} onChange={e => onUpdate({ ...section, caption: e.target.value })} placeholder="Caption (optional)..." />
          </div>
        );
      case 'video':
        return (
          <div className="space-y-2">
            <input className="w-full border rounded-lg px-3 py-2 text-sm" value={section.url || ''} onChange={e => onUpdate({ ...section, url: e.target.value })} placeholder="Video URL (YouTube/Vimeo)..." />
      ...
      case 'quote':
        return (
          <div className="space-y-2">
            <textarea className="w-full border rounded-lg px-3 py-2 text-sm" value={section.content || ''} onChange={e => onUpdate({ ...section, content: e.target.value })} placeholder="Quote text..." />
            <input className="w-full border rounded-lg px-3 py-2 text-sm" value={section.attribution || ''} onChange={e => onUpdate({ ...section, attribution: e.target.value })} placeholder="Author / Attribution..." />
          </div>
        );
      case 'gallery':
        return (
          <div className="space-y-2">
            <textarea className="w-full border rounded-lg px-3 py-2 text-sm" value={Array.isArray(section.images) ? section.images.join('\n') : ''} onChange={e => onUpdate({ ...section, images: e.target.value.split('\n').filter(Boolean) })} placeholder="One image URL per line..." rows={3} />
          </div>
        );
      case 'button':
        return (
      ...
      case 'list':
        return (
          <textarea className="w-full border rounded-lg px-3 py-2 text-sm min-h-[80px]" value={Array.isArray(section.items) ? section.items.join('\n') : ''} onChange={e => onUpdate({ ...section, items: e.target.value.split('\n').filter(Boolean) })} placeholder="One item per line..." rows={4} />
        );
      case 'table':
        return (
          <textarea className="w-full border rounded-lg px-3 py-2 text-sm font-mono min-h-[80px]" value={section.content || ''} onChange={e => onUpdate({ ...section, content: e.target.value })} placeholder="Markdown table or CSV..." />
        );
      default:
        return <p className="text-xs text-gray-400">Unknown section type</p>;
    }
  };
  ...
  const handleSave = async (publish = false) => {
    ...
    try {
      ...
      const payload = {
        ...
        author_name: authorName, hero_image: heroImage, tags, is_published: publish ? 1 : 0, is_featured: isFeatured ? 1 : 0,
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Button variant="ghost" size="sm" onClick={() => handleSave(false)} loading={saving}>Save Draft</Button>
          <Button size="sm" onClick={() => handleSave(true)} loading={saving}>Publish</Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          <input className="w-full border border-gray-200 rounded-xl px-4 py-3 text-lg font-bold focus:outline-none focus:ring-2 focus:ring-konkan-green-primary/20 focus:border-konkan-green-primary" value={title} onChange={e => setTitle(e.target.value)} placeholder="Blog post title..." />

          <textarea className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-konkan-green-primary/20 focus:border-konkan-green-primary resize-none h-20" value={excerpt} onChange={e => setExcerpt(e.target.value)} placeholder="Short excerpt / summary..." />

          <input className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-konkan-green-primary/20 focus:border-konkan-green-primary" value={heroImage} onChange={e => setHeroImage(e.target.value)} placeholder="Hero image URL..." />

          <input className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-konkan-green-primary/20 focus:border-konkan-green-primary" value={authorName} onChange={e => setAuthorName(e.target.value)} placeholder="Author name..." />
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
            ...
            <div>
              ...
              <select className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" value={categoryId} onChange={e => setCategoryId(e.target.value)}>
            ...
            <div>
              ...
              <input className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" value={tags} onChange={e => setTags(e.target.value)} placeholder="mango, recipes, konkan" />
            </div>
            <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={isFeatured} onChange={e => setIsFeatured(e.target.checked)} className="rounded" /> Mark as Featured</label>
          </div>

          {/* Import from File */}
          <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
            <h3 className="text-sm font-semibold text-gray-900">Import from File</h3>
            <p className="text-xs text-gray-500">Upload a PDF, DOCX, TXT, or Markdown file to auto-generate content.</p>
            <p className="text-xs text-blue-600 font-medium">Coming soon - file parsing integration</p>
          </div>
        </div>
      </div>
    </div>
