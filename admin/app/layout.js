...
const SIDEBAR_SECTIONS = [
  ...
  },
  {
    ...
    items: [
      ...
      { href: '/bank-offers', label: 'Bank Offers', icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 21h18M5 21V7l7-4 7 4v14M9 21v-6h6v6M9 10h.01M15 10h.01" /></svg> },
      { href: '/flash-sales', label: 'Flash Sales', icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg> },
      { href: '/bundles', label: 'Bundles', icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4M12 3v4m0 14v-3m0-3v-5" /></svg> },
      { href: '/campaigns', label: 'Campaigns', icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 1l2.4 7.6L22 11l-7.6 2.4L12 21l-2.4-7.6L2 11l7.6-2.4L12 1z" /></svg> },
    ],
  },
  {
    label: 'Content',
    items: [
      ...
      { href: '/customer-service', label: 'Customer Service', icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" /></svg> },
      { href: '/reviews', label: 'Reviews', icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.196-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" /></svg> },
      { href: '/hero-sliders', label: 'Hero Sliders', icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 15l4-4 3 3 5-6 4 4m1-4v9a2 2 0 01-2 2H5a2 2 0 01-2-2V6a2 2 0 012-2h9m-3 1a2 2 0 11-4 0 2 2 0 014 0z" /></svg> },
      { href: '/navbar', label: 'Navbar', icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 6h16M4 10h16M4 14h16M4 18h10" /></svg> },
    ],
  },
  {
    label: 'Inventory',
...
function AdminLayout({ children }) {
  ...
  if (!hydrated || !isAdminAuthenticated) {
    return (
      <div className="min-h-screen bg-konkan-earth flex items-center justify-center">
        <div className="w-8 h-8 border-2 border-konkan-saffron border-t-transparent rounded-full animate-spin" />
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-konkan-cream/80">
      {/* ══ Desktop Sidebar ══ */}
      <aside
        className={`fixed top-0 left-0 z-30 h-screen bg-konkan-earth transition-all duration-300 hidden lg:flex flex-col ${
      ...
      >
        ...
        <div className={`flex items-center h-16 px-4 border-b border-white/10 ${sidebarCollapsed ? 'justify-center' : 'justify-between'}`}>
          {!sidebarCollapsed && (
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-[9px] text-[#C9A88C] font-medium uppercase tracking-widest">Management</p>
              </div>
            </div>
          )}
          ...
          {!sidebarCollapsed && (
            <button onClick={() => setSidebarCollapsed(true)} className="p-1 rounded text-[#C9A88C]/60 hover:text-white hover:bg-white/5 transition-all">
          ...
          {sidebarCollapsed && (
            <button onClick={() => setSidebarCollapsed(false)} className="absolute -right-3 top-6 w-6 h-6 rounded-full bg-konkan-saffron/80 border-2 border-konkan-saffron/40 flex items-center justify-center text-white hover:text-white transition-all">
          ...
          {SIDEBAR_SECTIONS.map((section, si) => (
            <div key={si}>
              {section.label && !sidebarCollapsed && (
                <p className="px-3 mb-1.5 text-[9px] font-semibold text-[#C9A88C] uppercase tracking-[0.15em]">{section.label}</p>
              )}
                  ...
                  return (
                    <Link
                      ...
                      className={`flex items-center gap-3 px-3 py-2 rounded text-sm font-medium transition-all ${
                        isActive
                          ? 'bg-konkan-saffron/15 text-[#FFE3C2]'
                          : 'text-[#E8D8C3]/80 hover:text-white hover:bg-white/5'
                      } ${sidebarCollapsed ? 'justify-center px-2' : ''}`}
                      title={sidebarCollapsed ? item.label : undefined}
                    >
                      <span className={isActive ? 'text-[#FFE3C2]' : 'text-[#E8D8C3]/40'}>{item.icon}</span>
        ...
        <div className={`border-t border-white/10 p-3 ${sidebarCollapsed ? 'text-center' : ''}`}>
          <button
            onClick={adminLogout}
            className={`flex items-center gap-3 px-3 py-2 rounded text-sm font-medium text-[#C9A88C] hover:text-konkan-saffron hover:bg-konkan-saffron/10 transition-all w-full ${sidebarCollapsed ? 'justify-center' : ''}`}
      ...
      {/* Top navbar removed. Mobile-only floating menu button keeps the sidebar reachable. */}
      <button
        onClick={() => setMobileSidebarOpen(true)}
        aria-label="Open menu"
        className="lg:hidden fixed top-3 left-3 z-40 w-10 h-10 rounded-xl bg-konkan-earth text-white flex items-center justify-center shadow-lg active:scale-95 transition-transform"
      >
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {mobileSidebarOpen && (
        <div className="fixed inset-0 z-40 lg:hidden">
          ...
          <div className="absolute left-0 top-0 h-full w-72 bg-konkan-earth shadow-2xl">
            <div className="flex items-center justify-between h-16 px-4 border-b border-white/10">
              ...
              <button onClick={() => setMobileSidebarOpen(false)} className="p-1.5 rounded text-[#C9A88C]/60 hover:text-white hover:bg-white/5 transition-all">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <nav className="p-3 space-y-5 overflow-y-auto h-[calc(100%-64px)]">
              {SIDEBAR_SECTIONS.map((section, si) => (
                <div key={si}>
                  {section.label && <p className="px-3 mb-1.5 text-[9px] font-semibold text-[#C9A88C] uppercase tracking-[0.15em]">{section.label}</p>}
                  <div className="space-y-0.5">
                    {section.items.map((item) => {
                      const isActive = item.href === '/'
                        ? pathname === '/'
                      ...
                      return (
                        <Link
                          ...
                          className={`flex items-center gap-3 px-3 py-2.5 rounded text-sm font-medium transition-all ${
                            isActive ? 'bg-konkan-saffron/15 text-[#FFE3C2]' : 'text-[#E8D8C3]/80 hover:text-white hover:bg-white/5'
                          }`}
                        >
                          <span className={isActive ? 'text-[#FFE3C2]' : 'text-[#E8D8C3]/40'}>{item.icon}</span>
                          {item.label}
      ...
      <main className={`transition-all duration-300 ${sidebarCollapsed ? 'lg:ml-16' : 'lg:ml-64'} pt-0`}>
        <div className="p-4 lg:p-6 pt-16">{children}</div>
      </main>
    </div>
  );
}
